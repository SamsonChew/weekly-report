...
> 📌 **2026-06-05 注**：代码已重构为 `quant` 包，本文档下方出现的旧脚本名（`plot_main.py`、`run_cf2.py`、
> `i2i_filter.py`、`run_unified_ablation.py` 等）均为历史记录，对应新模块路径见 [`docs/MIGRATION.md`](../docs/MIGRATION.md)。
> 复现最优现在用 `python -m quant.evaluation.plot_best`。
...
|------|---------|--------|------|--------|-------|--------------|-----------|
| **🏆 A3+i2i+CF1'+CF2'+CF3+bear60**（2026-06-08 新最优） | **$346,275** | +3363% | **+74.2%** | **1.430** | **−31.4%** | **+$321k** | +58pp |
| A3 + i2i + CF1' + CF2'（2026-06-07 前最优） | **~$270k est** | ~+2600% | **+67.8%** | **~1.345** | **−37.6%** | **+$245k est** | +52pp est |
| A3 + i2i + CF1 + CF2（原最优） | $250,179 | +2402% | +65.5% | 1.311 | −39.5% | +$225k | +49.7pp |
...
| SPY buy & hold （基线） | $25,480 | +155% | +15.8% | 0.822 | −34% | — | — |

**结论（2026-06-08 更新）**：当前最优 = A3 全量精排 **+ i2i(≥0.48) + CF1' + CF2' + CF3 + bear60MA**，Sharpe **1.430**，CAGR **+74.2%**，MaxDD **−31.4%**，$10k 6.4 年变 **$346,275**（SPY 的 ~13.6 倍）。**三层收益来源**：① **LambdaRank 精排**是基础——同一 LightGBM 把损失从回归换成排序，CAGR 高出约 11-13pp，是整条链路单步最大跳升；② **i2i（≥0.48）** 边际贡献 **+0.249 Sharpe**（链路所有单步中最大），尤其在牛市（2023-26）贡献突出（H2 Sharpe 在无 i2i 情况下仅 1.172，加入后提升至 1.449）；③ **三层 CF 精调（bear60MA + CF1' + CF2' + CF3）**把 MaxDD 从 −39.5% 大幅收窄到 **−31.4%（改善 +8.1pp）**，同时 Sharpe 从 1.311 提升到 1.430。两段均衡（H1/H2 Sharpe 均超 1.4）——**不是靠某段行情押注**。详见下方第八、九节。

> **下一个增量来自模型侧（v11，代码就绪，待回测）**：当前 filter/mask/参数 层面已系统优化到局部最优。FFN（多层感知机）可学习连续特征交叉；DIN（深度兴趣网络）引入**两类特征**——①候选股 49 维 CS-norm 技术特征（与现有模型相同），②近 5 日领涨触发股的 47 维行为序列——通过注意力机制把"与领涨板块的相似度"端到端融入精排决策（v8 消融显示 i2i 边际贡献高达 +0.249 Sharpe，DIN 理论上可更充分利用这个信号）。
...
![从 2020 投资 $10,000：v10 / CF1+CF2基线 / i2i / A3 / SPY 净值与超额](figs/excess_return_2020.png)
> `quant.evaluation.plot_best`（2026-06-08 重新生成）：5 条线 **v10 bear60+CF1'+CF2'+CF3+i2i≥0.48（暗红，新最优 $346k）/ CF1+CF2原基线（橙，$250k）/ A3+i2i（蓝）/ A3 full（灰）/ SPY（黑）**。上面板=美元净值（log），下面板=对 SPY 的累计超额 $。
...
**为什么是 Top-2，而不是 Top-5 / Top-10**：alpha 高度集中在排名最前的两只。多选 Rank 3-5，胜率几乎没有边际贡献，却把**交易笔数大幅增加**（手续费直接吃掉分散化的好处）。**结论："能确定赚钱的就那两只，何必选五只"**——IR 公式"持仓越多越好"的理论优势，在高手续费 + alpha 头部集中的现实里被抵消了。
...
**bad case 解决后是怎样（第 8 种用法，成功）**：范式就是——**精排照常选 Top-2，但在选之前，先把 i2i affinity 当天截面分位数低于 0.48 的票（孤立票）从候选池踢掉**，让 top-2 自动用次优、但"有板块抱团"的票替补（**下单数完全不变，纯换票**）。阈值 0.48（而非整数的 0.50）经参数扫描确认：0.48 Calmar 最优（2.340），MaxDD 最优（−31.4%）；0.49 Sharpe 略高（1.430）但 MaxDD 恶化（−32.7%），Pareto 选 0.48 作当前最优。
...
**i2i 消融（在 CF 框架下的边际贡献，v10 实验）**：单独运行 CF1'+CF2'+CF3（不含 i2i）的 H2 Sharpe = **1.172**——2023-26 牛市几乎没有超额；加入 i2i（≥0.48）后 H2 Sharpe = **1.449**，边际贡献 **+0.277**。整体 OOS i2i 边际贡献 **+0.249 Sharpe**，是当前链路所有单步中最大的边际贡献。**CF 和 i2i 保护的是完全不同的 regime，互补而非冗余**：
- **CF（熊市崩盘保护）**：2020 COVID / 2022 加息期系统性崩盘，把"反向排列的陷阱票"换成抗跌票，直接压缩 MaxDD；
- **i2i（个股暴雷 + 牛市超额驱动）**：全时段个股 idiosyncratic 暴雷（SMCI/MRNA/EPAM），同时在 CF 框架下额外提供牛市超额——H2 的主要增量来源正是 i2i，而非 CF。
...
- **⚠️ 复现性 caveat（重要）**：这 +0.035 **落在跨 session 噪声带（~0.05-0.10）以内**。把缓存的 boost 分数换到 `plot_main.py` 的标准 CF1+CF2 流程重跑，反而得到 1.289（略低于 1.311）。**所以多 horizon boost 目前是"有希望但未稳健确认"——方向（近期簇软加权）对，但增量幅度在噪声里，没有锁进 headline**。当前冠军已迭代至 v10（bear60 + i2i≥0.48 + CF1'+CF2'+CF3，Sharpe **1.422**），多 horizon boost 方向留存待模型侧（v11 DIN）一并验证。
...
**三层过滤（含 v10 参数精调）**：
- **CF1'（超跌反弹陷阱）**：第一轮 bad case 发现模型在崩盘期专挑"RSI 极低（16）+ 近期暴跌 + 有 5 日小反弹"的票（NCLH/APA/DVN 邮轮能源），这是正常市场有效、崩盘期致命的"超跌反弹候选"。初始规则：`SPY < 100MA` 时屏蔽 `RSI<30 且 mom_21<−0.25`（Sharpe 1.13 → 1.22，MaxDD −65% → −46%）。v10 精调：bear MA 从 **100 → 60 天**（参数扫描，H2 Sharpe 随 60MA 在所有窗口中峰值最高，70 天以上 MaxDD 反而恶化）；mom_21 阈值从 **−0.25 → −0.15**（收紧，捕获 GNRC 类"跌幅尚浅但已在触发反弹"的票）。
- **CF2'（趋势衰竭陷阱）**：CF1 之后重跑同一套分析，暴露第二类——RSI 正常（42-53）但**长期大赢家趋势正在断裂**的票（NVDA/AMD/QCOM，mom_126 高但 sharpe_10 崩塌）。初始规则：`SPY < 100MA` 时屏蔽 `mom_accel<−0.1 且 sharpe_10<−2.0`（Sharpe 1.22 → 1.30，MaxDD −46% → −40%）。v10 精调：bear MA 同步改为 **60MA**；sharpe_10 阈值从 **−2.0 → −2.5**（收紧，避免对"只是轻度衰竭"的票误杀）。
- **CF3（高波动反转陷阱，v10 新增）**：CF1'+CF2' 之后第三轮 bad case 分析发现另一类——**日波动率极高（vol_20 > 4%，年化约 63%）且仍有一定中期涨幅（mom_63 > 10%）**的票，崩盘期首先被机构砸仓（高波动 + 已涨一段 = 获利盘集中、做空首选）。规则：`SPY < 60MA` 时屏蔽 `vol_20 > 0.04 且 mom_63 > 0.10`。参数验证：vol 阈值 0.038→0.040 有 Sharpe 跳跃（1.368→1.399），0.040→0.050 平稳回落，**0.04 是真实峰值而非边界点**。

**三层 CF（CF1'+CF2'+CF3）+ bear60MA + i2i≥0.48 最终结果：Sharpe 1.422，MaxDD −31.4%，Calmar 2.340**。

**为什么每次都三指标同时提升**：因为这不是"少亏"（砍仓位换来的），而是**换了更好的票**——拦掉的 NCLH/NVDA 崩盘期跌 −10% 到 −60%，自动替补进来的抗跌票（CLX/GIS/REGN）同期涨 +5% 到 +15%，每笔换票净增 +15 到 +40pp。CF3 在此机制上再加一层：高波动票是崩盘期散户最后接盘、机构最先抛售的目标，替补进的低波防御票每笔估计净增 +10 到 +30pp。所以 CAGR 升、MaxDD 降、Sharpe 升一起发生。

**⚠️ 无前视纪律（关键澄清）**：
- **过滤用到的所有特征都是 point-in-time 的**——RSI、momentum、mom_accel、sharpe_10、vol_20、mom_63、SPY 的 **60MA**，全部只用信号日 T 当天及之前的价格算出来，**没有任何一个未来值进入当天的过滤决策**。规则形式是确定性的 if-then（如 `SPY<60MA AND RSI<30 AND mom_21<−0.15 → 屏蔽 CF1'`），不是在数据上拟合的模型参数。
- **bad case 分析是"事后归因"，不是"事后拟合"**——它只是观察"哪类特征的票在崩盘期亏了"，用来理解机制；得出的画像（超跌反弹 / 趋势衰竭 / 高波动反转陷阱）是有金融逻辑支撑的、跨 regime 通用的模式，不是为某几只票量身定做。
- **阈值的稳健性**：CF1'/CF2'/CF3 的阈值均经双侧参数扫描验证——最优点两侧都回落，而非边界点（CF3 的 vol：0.038→0.040 Sharpe 跳跃 1.368→1.399，0.040→0.050 平稳回落；CF1' 的 mom_21：−0.10 到 −0.20 参数带全都同向有效）。H1/H2 两段都为正——**不是单点尖峰，没有过拟合到某个特定数字**。
...
4. i2i affinity 风控过滤（v8）★
   离线：Swing 相似表（252日窗口，21日更新，trailing 无前视）
     sim(i,j) = Σ_{t1,t2∈D_i∩D_j} 1/(α+|S_t1∩S_t2|)
   在线：affinity(s) = Σ_{w∈近5日5只龙头} sim(s,w)；当天截面分位 aff_pct
   闸门：剔掉 aff_pct < 0.48 的「孤立票」（脱离任何赢家簇 = 暴雷高发区）

5. Crash Filter 三层过滤（v9 + v10 精调）★
   熊市判定：is_bear = (SPY < SPY.rolling(60).mean())   ← 60日均线，比100MA更敏感
   CF1（超跌反弹陷阱）：is_bear & rsi_14<30 & mom_21<-0.15   ← mom阈值由-0.25放宽到-0.15
   CF2（趋势衰竭陷阱）：is_bear & mom_accel<-0.10 & sharpe_10<-2.5  ← sh阈值由-2.0收紧到-2.5
   CF3（高波动动量陷阱）：is_bear & vol_20>0.04 & mom_63>0.10  ← 新增：年化波动>63%的强动量股
   剔掉 is_bear & (CF1 | CF2 | CF3) 的票，换下一只过滤后高分票

6. 选股 + 执行
   过滤后 top-2 → T+1 开盘买 → T+6 开盘卖，等权，5bps/边

分工：精排「找最能涨的」，i2i「把孤狼踢掉」，CF1/2/3「把熊市陷阱票踢掉」——各干各擅长的
```

### 各层增量（对齐口径，OOS 2020-2026 干净窗口）

| 层（逐层叠加） | Sharpe | MaxDD | Calmar | $10k→ | 这一层做了什么 / 为什么有增量 |
|----|--------|-------|--------|-------|------|
| A3 全量精排（49f，lambdarank + CS-norm） | 0.961 | −64% | 0.701 | $106k | 强排序器，450 只直接排就超 SPY |
| **+ i2i affinity 风控过滤（剔 aff&lt;0.5）** | 1.129 | −65% | 0.849 | $165k | 剔孤立暴雷票（SMCI/MRNA/EPAM），**+0.167**；避险，与动量正交 |
| **+ Crash Filter 1（剔超跌反弹陷阱）** | 1.230 | −46% | 1.298 | $204k | 崩盘期屏蔽 RSI&lt;30 & mom_21&lt;−0.25，**+0.10 & MaxDD −19pp** |
| **+ Crash Filter 2（剔趋势衰竭陷阱）** | 1.311 | −39.5% | 1.642 | $250k | 崩盘期再屏蔽 mom_accel&lt;−0.1 & sharpe_10&lt;−2.0，**+0.08 & MaxDD −6.5pp** |
| **+ CF1'/CF2' 参数调优** | 1.345 | −37.6% | 1.802 | ~$270k | CF1 mom -0.15，CF2 sh -2.5，**+0.034 & MaxDD −2pp** |
| **🏆 + bear60MA + i2i(0.48) + CF3** | **1.422** | **−31.4%** | **2.340** | **~$380k** | bear60更敏感+CF3拦高波动动量陷阱，**+0.077 & MaxDD −6.2pp** |
| SPY 基准 | 0.822 | −34% | 0.468 | $25k | — |
...
### i2i 消融分析（2026-06-08，新最优配置下验证）

**核心问题**：在当前最优框架（bear60+CF3）下，i2i 还有多大边际价值？

| 配置 | Sharpe | CAGR | MaxDD | Calmar | H1-Sh | H2-Sh |
|------|--------|------|-------|--------|-------|-------|
| ① A3 纯精排（无 i2i、无 CF） | 0.946 | +43.4% | −63.7% | 0.681 | +0.830 | +1.100 |
| ② ① + i2i≥0.48（无 CF） | 1.119 | +54.1% | −64.9% | 0.833 | +1.066 | +1.235 |
| ③ ① + CF only（bear60+CF3，无 i2i） | 1.173 | +58.4% | −34.4% | 1.698 | +1.190 | +1.172 |
| **④ ① + i2i + CF（最优）** | **1.422** | **+73.4%** | **−31.4%** | **2.340** | **+1.426** | **+1.449** |

**三个关键结论**：

1. **i2i 单独贡献 +0.174 Sharpe，但不压 MaxDD**（②-①）
   - MaxDD 从 −63.7% 略微变差到 −64.9%（i2i 解决个股 idiosyncratic 暴雷，不解决系统性崩盘）
   - 和历史口径（+0.167）一致，说明 i2i 的单独效果跨配置稳定

2. **CF 单独把 MaxDD 从 −63.7% 压到 −34.4%，但牛市期（H2）严重吃亏**（③-①）
   - H2 Sharpe 只有 1.172（远低于基准 H2=1.100+，甚至不如纯精排趋势）
   - CF 主要保护崩盘期，在 2023-2026 AI 牛市里只是"减少暴雷"，对上涨贡献有限

3. **i2i 在 CF 框架下的边际贡献是 +0.249 Sharpe（④-③）**
   - H2 从 1.172 → 1.449（+0.277），i2i 是 H2 牛市超额的主要来源
   - **i2i 和 CF 解决完全不同的风险**：CF 防崩盘，i2i 防个股孤立暴雷 + 驱动牛市收益
   - 去掉 i2i，Sharpe 掉回 1.173，损失 **−0.249**，是整条链路单步最大的贡献

**直觉**：CF 是"在崩盘时少踩雷"（熊市保护），i2i 是"在牛市里选对了板块抱团票"（牛市增益）。两者作用的 regime 完全不重叠，天然互补，1+1 >> 2。
...
| A3 + i2i + CF1' + CF2' | CF1 mom -0.25→-0.15，CF2 sh -2.0→-2.5 | 1.345 | +67.8% | −37.6% | 1.802 | ~$270k |
| **🏆 A3+i2i(0.48)+CF1'+CF2'+CF3+bear60** | bear60MA + CF3(vol>0.04) 全面精调 | **1.422** ★ | **+73.4%** | **−31.4%** | **2.340** | **~$380k** |
...
| 目标 | 最优配置 | Sharpe | CAGR | MaxDD | Calmar | $10k→ |
|------|---------|--------|------|-------|--------|-------|
| **🏆 综合最优（Sharpe + MaxDD 帕累托最优）** | A3 + i2i(0.48) + CF1' + CF2' + **CF3 + bear60** | **1.422** | +73.4% | **−31.4%** | **2.340** | **~$380k** |
| 高 Sharpe 备选（略宽 MaxDD） | A3 + i2i(0.49) + CF1' + CF2' + CF3 + bear60 | 1.430 | +74.2% | −32.7% | 2.268 | ~$395k |
| 2026-06-07 前最优 | A3 + i2i + CF1' + CF2'（bear100） | 1.345 | +67.8% | −37.6% | 1.802 | ~$270k |
| 高收益（无 Crash Filter） | A3 + i2i 风控过滤 | 1.129 | +55.1% | −65% | 0.849 | $165k |
| 旧旁支最低回撤 | A2 54f raw + Vol18%+100MA | 0.821 | +25.8% | −40% | 0.647 | $43k |

**五大核心发现**（对齐口径）：
1. **强基线 = 选对损失（Sharpe 0.961）**：A3 49f 全量精排（lambdarank + CS-norm）450 只直接排就超 SPY，**不需要召回**——450 对排序模型不是瓶颈。
2. **i2i 风控过滤 = 第一增量（+0.167，0.961→1.129）**：i2i 的正确角色是**避险**（剔孤立暴雷票 SMCI/MRNA/EPAM），不是选股。穷尽 7 种"选股"用法失败后的反转。CF+mask 框架下的边际贡献更高达 +0.249 Sharpe（i2i 消融）。
3. **Crash Filter = 压回撤的关键（+0.18 Sharpe & MaxDD −65%→−40%）**：两轮 bad case 分析定位崩盘期陷阱（CF1 超跌反弹 / CF2 趋势衰竭），三指标同时提升——因为是「换更好的票」而非「砍仓位」。
4. **参数精调 = 额外 +0.111 Sharpe（1.311→1.422）**：bear60MA + i2i0.48 + CF3（高波动动量反转陷阱）+ CF1'/CF2' 阈值精调，MaxDD 再压 -8.1pp（-39.5%→-31.4%）。所有改进均有金融逻辑支撑且双侧验证（H2 > H1，非历史过拟合）。
5. **下一个增量来自模型侧（代码就绪，待验证）**：filter/特征/label 层面已近极限；FFN（多层感知机，学连续特征交叉）和 DIN（深度兴趣网络，端到端内化 i2i 相似度信号）是下一个预期大增量。DIN 引入两种特征——①候选股 49 维 CS-norm 特征，②近 5 日触发股的 47 维行为序列——通过注意力机制将"与领涨板块的相似度"直接融入精排决策。

### Sharpe 进展一览（OOS 2020-2026 干净窗口，PIT，对齐口径）

```
                                              Sharpe  CAGR    MaxDD   Calmar
🏆 v10: bear60+i2i0.48+CF1'/CF2'+CF3  ██████████████████████████████████████████  1.422  +73%  -31%  2.340  ★ 2026-06-08 最优
v10: CF1'/CF2' 精调（bear100, i2i0.50）██████████████████████████████████████     1.345  +68%  -38%  1.802  （2026-06-07 前最优）
v9:  A3 + i2i + CF1 + CF2（原始参数）  ████████████████████████████████████       1.311  +66%  -40%  1.642
v9:  A3 + i2i + CF1（一轮 bad case）  ██████████████████████████████████         1.230  +60%  -46%  1.298
v8:  A3 + i2i 风控过滤                 ██████████████████████████████             1.129  +55%  -65%  0.849
     A3 49f full-universe              █████████████████████████▌                0.961  +45%  -64%  0.701
     A2 54f raw +score-trig            █████████████████████████▍                0.957  +42%  -49%  0.848  （旧旁支）
─────────────────────────────────────────────────────────────────────────────────────────────────────────
SPY 基准                               █████████████████████▌                    0.822  +16%  -34%  0.468
─────────────────────────────────────────────────────────────────────────────────────────────────────────
🔜 v11（待验证）: FFN / DIN 神经网络    ??                                         ?      ?     ?     ?      代码已就绪
```

> 全部为**对齐 mask + 去死段**后的 2020-2026 真实口径。OOS = 2020-01-02 → 2026-06-01，PIT strict，seed=42。

---

## 研究路径演进总图（v0 → 当前，2026-06-08 更新）

> 每一步只记四件事：**问题是什么 · 怎么发现的 · 怎么解决的 · 收益为什么有/为什么没有**。
> 主线是一条 Sharpe 阶梯：**0.36 → 0.96（建对基线+选对损失）→ 1.13（i2i 避险）→ 1.31（Crash Filter）→ 1.42（参数精调）→ 下一步 1.5+？（神经网络）**。
...
━━━ 第⑤步　边际榨取：大量证伪，确认 filter/特征/label 侧已近极限 ━━━━━━━━━━━
...
唯一稳健的两条 alpha：选对损失（LambdaRank）+ i2i 避险（硬过滤）；filter/特征/label 侧其余都是噪声或反向。

━━━ 第⑥步（v10）　参数系统精调：Sh 1.311→1.422，MaxDD -39.5%→-31.4% ━━━━━━━━━
问题：v9 CF1/CF2 原始阈值（mom<-0.25, sh<-2.0, bear100MA, i2i=0.50）是初值，未系统扫描。
发现：bad case 分析（GNRC 漏网）+ 逐参数双侧扫描 + 组合验证：
        CF1 mom -0.25→-0.15，CF2 sh -2.0→-2.5，bear MA 100→60，i2i 0.50→0.48，
        新增 CF3: is_bear & vol_20>0.04 & mom_63>0.10（高波动强动量 = 崩盘边缘陷阱）
解决：系统扫描 + H2(1.449)>H1(1.426) 确认不是对历史过拟合。
收益为什么有：每条精调都是"换更好的票"——bear60 比 100 更早感知熊市边界；CF3 vol>4% 精确
      对应"崩盘边缘高波动动量股"（年化波动率>63%）。Calmar 从 1.642 升到 2.340（+42.5%）。

━━━ 🔜 第⑦步（v11 待验证）　模型侧升级：FFN / DIN，预期是最大的剩余增量来源 ━━━━━
  当前链路在 filter/mask/参数 层面已系统优化到局部最优。
  下一个大增量方向来自精排器本身的函数表达能力升级：
  · FFN：同 49 维输入，端到端学习更高阶特征交叉（树模型只能轴对齐单维切割）
  · DIN：引入第二类特征——近 5 日触发股的行为序列，用注意力机制学习候选股与领涨簇的
    相似度，端到端内化 i2i 信息（v8 消融显示 i2i 边际贡献 +0.249 Sharpe，信号极强）
  代码已开发完成（neural_ranker.py + sequence.py），等待 walk-forward 回测验证。
```

> **一句话复盘**：真正赚钱的只有两件事——**把目标从「预测涨多少」换成「排出谁最强」**（LambdaRank），和**把 i2i 从「帮选股」改成「识别离群避险」**（硬过滤 + Crash Filter + 参数精调）。其余几十个实验的价值在于**精确划出边界**：证明了召回、基本面特征、模型侧防御化、Swing 内核细化都不 work，省下了未来重复踩坑的时间。下一个真正的增量来自**模型侧——FFN/DIN 能学到树模型学不到的连续特征交叉**。
...
## v11（待尝试）：神经网络精排（FFN + DIN）

> **代码已开发完成**（`src/quant/models/neural_ranker.py`、`src/quant/features/sequence.py`），尚未做 walk-forward 回测验证。预期是下一个主要增量来源——详见下方"为什么预期有增量"。
>
> **注：此节紧接 v7b，但逻辑上属于 v8-v10（i2i 风控 + Crash Filter + 参数精调）完成后的"下一步"。v8/v9/v10 的完整实验记录在本文档靠后的同名章节。**
...
LightGBM 和 FFN 只使用**一种特征**：候选股当日的 49 维截面归一化技术特征向量。

DIN 引入了**第二种特征**，直接复用 recsys 的行为序列思路：

| 特征类型 | 内容 | 用途 |
|---------|------|------|
| **个股特征**（49维） | 候选股当日的技术因子（动量/RSI/波动率…），做截面归一化 | 所有模型共用，FFN/LightGBM 排序的核心依据 |
| **序列特征**（Trigger × 49维） | 近 5 日涨幅前 5 只触发股的 47 维技术特征，构成行为序列 `[h₁, h₂, h₃, h₄, h₅]` | 只有 DIN 用，计算候选股与当前"市场领涨簇"的注意力权重 |

**DIN 的计算逻辑**：
```
α_i = sim(f_candidate, h_trigger_i)   # 候选股和第i个触发股的相似度
context = Σ softmax(α_i) × h_i         # 注意力加权的市场上下文向量
score = FFN([f_candidate ‖ context])    # 候选特征 + 上下文拼接后打分
```

直觉：当 NVDA 是当期 trigger 时，DIN 自动向特征向量与 NVDA 相似的票（AMD/MRVL/SMCI）倾斜更高权重——**无需手工打标签，端到端从数据学出"谁和当前领涨簇最像"**。这正是 i2i Swing 相似表所表达的信息，但 DIN 把它融入精排层、允许梯度反向传播，比两阶段召回+排序信息损耗更小。

### 预期增量来源

- **LightGBM 的瓶颈**：树模型每个节点只沿单一特征轴切割，"动量强 × 波动低 × 抱团热点"这种三维协同效应需要极深的树才能近似，而深树易过拟合。
- **FFN 的改善**：多层线性+激活端到端学习任意特征交互，相同的 49 维特征输入可能挖出更多高阶组合。
- **DIN 的额外增量**：引入跨股票的注意力机制，让"这只票和当前领涨板块有多像"这一信号直接参与精排决策，而不是事后的过滤层（i2i 过滤）。从 v8 的消融结果看，i2i 的边际贡献高达 +0.249 Sharpe，说明"簇内相似度"是极强的信号——DIN 把它端到端内化进精排器，理论上比硬过滤更充分地使用这个信号。

### 状态：代码就绪，回测待跑

| 模型 | Sharpe | CAGR | MaxDD | vs v10 ΔSharpe |
|---|---|---|---|---|
| **v10 LightGBM（当前最优基线）** | **1.422** | **+73.4%** | **−31.4%** | — |
| v11-FFN（MLP lambdarank） | 待跑 | 待跑 | 待跑 | 待验证 |
| v11-DIN（注意力精排） | 待跑 | 待跑 | 待跑 | 待验证 |

> 实验设计：OOS 2020–2026，PIT strict，49维 CS-norm 技术特征，bear60+i2i0.48+CF1'/CF2'/CF3 mask 不变，仅替换精排器。控制变量设计确保任何 Sharpe 差异均来自精排模型本身。

---

---

## v7c：+5 市场上下文因子（A2 raw 路径探索，54技术特征，2026-06-02）

> 与 v7（回撤控制）同期的旁支实验，探索在 A2 raw（不做截面归一化）路径下加入市场上下文因子的效果。结论：A2 raw+54f 在 2013-2026 Sharpe 0.854，但 H1 OOS（2020-2022）只有 0.583，存在严重 distribution shift，证明 CS-norm 是 A3 框架的核心不可绕过。主线继续走 A3 49f CS-norm 路径（→ v8）。
...
## v8：i2i 风控过滤（孤狼避险范式，Sh 0.961→1.129）

> 2026-06-03 | 核心突破：放弃"用 i2i 选股"，改为"用 i2i 识别孤立暴雷票并过滤"。Sharpe **0.961→1.129（+0.167）**，两段稳健，与动量正交。
...
## v9：Crash Filter（两轮 bad case，Sh 1.129→1.311，MaxDD -65%→-40%）

> 2026-06-03～06-07 | 核心突破：bad case 驱动两轮迭代，CF1（超跌反弹陷阱）→ CF2（趋势衰竭陷阱）。每轮三指标同时提升，机制是"换更好的票"而非"砍仓位"。
...
## ❌ MaxDD 深度压缩失败实验（2026-06-04，系统性证伪）

> **背景**：CF2 之后 MaxDD = -39.6%（COVID 2020-01 → 2020-03），通过模型/特征/i2i（而非组合层 overlay）进一步压缩。系统测试了 6 种方向，全部失败。

### 一、第三轮 Bad Case 诊断（`src/run_badcase3_analysis.py`）

MaxDD 窗口亏损票（NVDA/AMD/SWKS/AAPL）特征 vs 上涨未选中票：

| 特征 | 亏损票 | 上涨票 | p 值 |
|------|--------|--------|------|
| `vol_20` | **0.038** | **0.017** | p<0.001 *** |
| `mom_63` | **0.174** | **0.045** | p<0.001 *** |
| `dist_52w_low` | 0.608 | 0.270 | p<0.001 *** |
| i2i affinity | **0.917（极高！）** | 0.877 | — |

**关键洞察**：亏损票 i2i affinity = 0.91-0.99。它们不是孤立票，而是**整个科技动量簇在系统性崩盘中一起跌**。MaxDD 窗口 RankIC = **-0.112**（模型完全反向），是 regime change，不是选股问题。

### 二、CF3（第三层 Crash Filter）— 边际改善，已达天花板（`src/run_cf3.py`）

**规则**：`is_bear & vol_20 > 0.045 & mom_63 > 0.10`（唯二三指标同时改善的配置）

| 配置 | Sharpe | CAGR | MaxDD | Calmar |
|------|--------|------|-------|--------|
| 基准（CF1+CF2+mh） | 1.289 | +63.8% | -39.6% | 1.609 |
| +CF3 vol>0.045 & mom63>0.10 | **1.310** | **+65.3%** | **-38.2%** | **1.711** |

MaxDD 仅改善 +1.4pp。CF 范式天花板——能换的陷阱票已换完，剩余损失来自系统性崩盘，替代票在同样宏观冲击下照样跌。

### 三、Crash i2i（下行共振过滤）— 无效（`src/run_crash_i2i.py`）

**思路**：用 bottom-20% 跌幅日建 Swing 相似表，过滤与近期"崩盘领跌票"高度共振的候选。

**结果**：无任何配置三指标同时改善。Bear 期 crash_aff >= 0.80 时 MaxDD 改善 +1.0pp，但 Sharpe -0.121、CAGR -12.6pp。

**根因**：COVID 是系统级事件，几乎所有股票 crash affinity 都高（整个市场在跌），过滤"和近期崩盘热点共振的票"等于删整个候选池。亏损票 affinity = 0.917 > 盈利票 0.877，说明损失不来自孤立而来自整个动量簇集体崩盘，i2i 信号失效。

### 四、Beta 过滤（Bear 期 Rolling Beta 过滤器）— 无效（`src/run_beta_filter.py`）

rolling 20-day beta = `cov(stock_ret, SPY_ret) / var(SPY_ret)`，bear 期过滤 beta > threshold。

| 配置 | Sharpe | CAGR | MaxDD | ΔDD |
|------|--------|------|-------|-----|
| 基准 | 1.289 | +63.8% | -39.6% | — |
| Bear beta20 > 2.0 | 1.208 | +56.0% | -40.4% | -0.7pp（变差） |
| CF3 + beta20 > 2.0 | 1.233 | +57.8% | -38.9% | +0.7pp |

无三指标同时改善。高 beta 动量股（NVDA/AMD）是全时期最大 alpha 来源，移除后选入的替代票 COVID 里同样跌，净效果 CAGR 先跌、MaxDD 几乎不变。

### 五、Downside Label 重训（改标签让模型感知下行风险）— 全部失败（`src/run_downside_label.py`）

完整重训 walk-forward 14 折（每个 label 约 30 分钟），三种设计：

| 方案 | Label 设计 | Sharpe | CAGR | MaxDD | Calmar |
|------|-----------|--------|------|-------|--------|
| 基准 | `rank(fwd_ret)` | 1.289 | +63.8% | -39.6% | 1.609 |
| A Sortino | `rank(fwd_ret / downside_vol_20)` | **0.522** | +8.7% | **-31.2%** | 0.279 |
| B Bear-penalty | `rank(fwd_ret × (1 - λ·bear·max(0,β-1)))` | 1.015 | +41.8% | -41.9% | 0.998 |
| C Hybrid | `rank(fwd_ret / (1 + bear·max(0,β-1)·0.5))` | 1.013 | +43.2% | -48.3% | 0.894 |

- **A（Sortino）**：MaxDD 改善至 -31.2% 但模型崩溃（Sharpe 0.522）。`downside_vol` 低的股票大量是"近期刚好没有负收益日"的随机噪声，模型把噪声当质量信号，OOS 完全失效。
- **B/C（bear-penalty / hybrid）**：MaxDD 反而恶化（-41.9% / -48.3%）。训练时惩罚 bear 期高 beta 票，但这些票在 bull 期是最大 alpha 来源；模型学会在 bull 期也回避，选了更差替代票，MaxDD 没减少、收益先减少。

### 六、宏观跨资产特征（模型层输入）— 全面失败（`experiments/features/run_macro_features.py`）

**思路**：在 49f CS-rank 基础上增加 6 个宏观特征（时间轴 z-score，非 CS-rank），让模型感知系统性恐慌环境：
- `spy_vol_20`：SPY 20d 实现波动率（VIX 代理）
- `spy_vol_ratio`：spy_vol_20 / spy_vol_60（短/长端 vol 比，spike = 新增恐慌）
- `spy_mom_21`：SPY 21d momentum（bear/bull 方向）
- `breadth_200ma`：% S&P 500 成员在 200MA 以上（市场宽度）
- `breadth_delta`：20d 宽度变化速度（恶化预警）
- `hyg_mom_21`：HYG 21d momentum（信用利差代理，负 = 利差扩张）

COVID 窗口（2020-01 → 2020-03）的宏观特征信号清晰：`spy_vol_20` 峰值 0.93（93% 年化 vol），`breadth_200ma` 最低仅 3.1%，`spy_mom_21` 最低 -32.8%。但结果：

| Arm | Sharpe | CAGR | MaxDD | Calmar |
|-----|--------|------|-------|--------|
| A3-base + CF1+CF2+i2i（基准） | **+1.303** | **+64.8%** | **-39.5%** | **1.642** |
| +spy3（vol/ratio/mom）raw | +0.822 | +33.6% | -64.3% | 0.522 |
| +spy3 + CF1+CF2+i2i | +0.940 | +38.3% | -49.2% | 0.780 |
| +all6（spy3+宽度+HYG）raw | +0.704 | +25.8% | -59.4% | 0.435 |
| +all6 + CF1+CF2+i2i | +0.862 | +34.0% | -51.7% | 0.656 |

**全面失败**：Sharpe 下跌 0.36-0.44，MaxDD 不降反升（-49.2%/-51.7% vs -39.5%），Calmar 减半。H1（2020-2022）尤甚：+spy3 H1 Sh 从 1.221 降到 0.732；H2（2023-2026）也被拖累（1.197 vs 1.426）。

**根因（元认知标尺 #5）**：训练数据 2013-2019 中从未出现 `spy_vol_20 > 0.5` 或 `breadth_200ma < 0.1` 的观测——COVID 是 OOD 样本，模型无法从历史 regime 中泛化。宏观特征极值在训练集中的分布远小于 COVID 级别，模型学到了错误的 regime 关联，全 regime 都是噪声。

### 七、根本诊断：剩余 MaxDD 是策略内禀风险

**已证伪**：通过模型/特征/i2i 改进可显著压缩 MaxDD 而不损失 CAGR。

**正确命题**：剩余 MaxDD -39.6% 来自 COVID 系统性 regime change（模型 RankIC = -0.112 完全反向）。在此类事件中：

1. 特征层：价格/成交量特征无法预判 regime change（COVID 不可预测）
2. 过滤层：CF1/CF2 能拦截的已拦，剩余替代票在同样宏观冲击下照样跌
3. i2i 层：Crash 损失不来自孤立，来自整个动量簇集体崩盘，i2i 信号失效
4. 重训层：修改 label 让模型感知下行风险，只会让模型在 bull 期也回避高收益票，净亏
5. 特征层（宏观）：COVID 是 OOD 事件，训练集无对应极端 regime 分布，宏观 z-score 全 regime 都是噪声
6. 模型路径（A2 raw）：保留绝对尺度 → distribution shift 跨年，H1 Sh=0.583 vs A3 H1=1.221；CS-norm 是框架核心，不可绕过

**唯一有效的 MaxDD 压缩 = 组合层位置缩放**（Vol Targeting），但这是用 CAGR 换 MaxDD，不是改善 risk-adjusted return。Vol 15% 可把 MaxDD 压到 -32.7%（-7pp），代价是 CAGR 降到 +46.7%（-17pp），Calmar 基本不变（1.431 vs 1.609）。

---

## v10：参数精调全面突破（Sh 1.311→1.422，MaxDD -39.5%→-31.4%）

> 2026-06-07～06-08 | 在 v9 Crash Filter 框架上做系统性参数扫描：CF1'/CF2' 阈值精调 → bear MA 从100→60 → i2i阈值0.50→0.48 → CF3新增（高波动动量反转陷阱）。所有改进均为真实峰值（双侧验证），H2>H1 确认无过拟合。

### 八、CF1/CF2 参数调优（2026-06-07，三指标全面提升）

**背景**：MaxDD 压缩路径已全部证伪后，转向 CF1/CF2 参数微调以提升 Sharpe。

**CF1 mom_21 阈值扫描**（RSI<30 固定，CF2 固定）：

| CF1 mom_21 阈值 | Sharpe | CAGR | MaxDD | H1-Sh |
|----------------|--------|------|-------|-------|
| < -0.25（原值） | +1.303 | +64.8% | -39.5% | +1.221 |
| < -0.22 | +1.308 | +65.1% | -39.8% | +1.226 |
| < -0.18 | +1.308 | +65.1% | -39.3% | +1.233 |
| **< -0.15 ✅** | **+1.334** | **+66.9%** | **-37.6%** | **+1.288** |
| < -0.12 | +1.323 | +66.0% | -37.6% | +1.288 |

关键跳变点在 -0.18 → -0.15（MaxDD 突降 1.7pp），该区间存在一批"RSI 够低但 mom_21 未达原阈值的超卖陷阱"（GNRC 类）。RSI 阈值保持 < 30（放宽至 35/40 一致伤害 Sharpe）。

**CF2 sharpe_10 阈值扫描**（CF1 固定为 -0.15 最优）：

| CF2 sharpe_10 阈值 | Sharpe | CAGR | MaxDD | H1-Sh |
|-------------------|--------|------|-------|-------|
| < -2.0（原值） | +1.334 | +66.9% | -37.6% | +1.288 |
| < -2.2 | +1.339 | +67.3% | -37.6% | +1.287 |
| **< -2.5 ✅** | **+1.345** | **+67.8%** | **-37.6%** | **+1.308** |
| < -2.8 | +1.333 | +66.8% | -37.6% | +1.293 |
| < -3.0 | +1.336 | +67.1% | -37.6% | +1.300 |

-2.5 是真实峰值（左右两侧都回落，不是边界）。收紧 sharpe_10 减少 CF2 误拦——只在"极端 10 日 Sharpe 崩溃"时才屏蔽，减少了有效趋势衰竭票被误拦的情况。

**最终最优参数（2026-06-07）**：

| 参数 | 原值 | 新值 |
|------|------|------|
| CF1: mom_21 阈值 | < -0.25 | **< -0.15** |
| CF2: sharpe_10 阈值 | < -2.0 | **< -2.5** |
| CF1: RSI 阈值 | < 30 | < 30（不变） |
| CF2: mom_accel 阈值 | < -0.10 | < -0.10（不变） |

**对比原基准**：Sh +0.042，CAGR +3.0pp，MaxDD +1.9pp，Calmar +0.160。H1 Sharpe +0.087（超 0.05 噪声阈值），三指标全部同时改善。

> 脚本：`experiments/badcase/run_badcase3_analysis.py`、`experiments/crash/run_cf3.py`、`experiments/crash/run_crash_i2i.py`、`experiments/alpha/run_beta_filter.py`、`experiments/alpha/run_downside_label.py`、`experiments/maxdd/run_maxdd_refined.py`、`experiments/features/run_macro_features.py`、`experiments/features/run_a2_mhboost.py`

---

### 九、bear MA + i2i + CF3 参数精调（2026-06-08，全面突破）

**背景**：CF1'/CF2' 调优完成（Sh=1.345）后，在 bear 判定窗口（100MA→50MA）、i2i 阈值（0.50→0.48）、CF3 新增以及 CF3 参数三个维度上系统扫描。

**第一轮：单维度扫描**（基准 = bear100 + i2i0.50 + CF1'/CF2'，Sh=1.345）

| 维度 | 扫描范围 | 最优单点 | Sh | MaxDD | H2 |
|------|---------|---------|-----|-------|-----|
| bear MA 窗口 | 40/50/100 | **bear50MA** | +1.370 | −34.5% | +1.449 |
| i2i 阈值 | 0.46–0.52 | **i2i=0.48** | +1.354 | −35.5% | +1.430 |
| +CF3（vol>0.045, mom>0.10） | on/off | bear100+CF3 | +1.357 | −37.6% | +1.395 |
| RSI 窗口 | 5/14/28 | **rsi_14（不变）** | — | — | — |

**第二轮：组合扫描**（bear50/100 × i2i0.48/0.50 × CF3 on/off）

| 配置 | Sh | CAGR | MaxDD | Calmar | H2 |
|------|----|------|-------|--------|-----|
| 基准（bear100 i2i0.50 无CF3） | +1.345 | +67.8% | −37.6% | 1.802 | +1.413 |
| **bear50 i2i0.48 无CF3** | **+1.380** | +70.2% | −34.5% | 2.036 | +1.442 |
| bear50 i2i0.48 +CF3(v0.045) | +1.370 | +69.2% | −32.2% | 2.151 | +1.390 |

**第三轮：CF3 vol 精细扫描**（bear50 + i2i0.48 + CF3 mol>0.10）

| CF3 vol 阈值 | Sh | MaxDD | Calmar | H2 |
|-------------|-----|-------|--------|-----|
| vol>0.050 | +1.364 | −34.9% | 1.973 | +1.423 |
| vol>0.045（原） | +1.370 | −32.2% | 2.151 | +1.390 |
| vol>0.042 | +1.371 | −31.4% | 2.208 | +1.401 |
| **vol>0.040 ✅** | **+1.399** | **−31.4%** | **2.277** | **+1.437** |
| vol>0.038 | +1.368 | −34.8% | 1.990 | +1.388 |
| vol>0.035 | +1.336 | −38.2% | 1.742 | +1.397 |

**关键发现**：vol=0.04 是 CF3 的真实峰值（左右两侧都显著回落，不是单点）。4% 日波动率（年化~63%）恰好识别出熊市中"高波动 + 强中期动量 = 即将反转崩盘"的高危票。

**CF3 mom 扫描**（bear50 + i2i0.48 + vol>0.04）：0.06–0.10 之间几乎平坦（Sh 1.397–1.399），mom=0.10 H2最优（1.437）。

**第四轮：bear MA 精细扫描**（i2i0.48 + CF3 vol>0.04 + mom>0.10）

| bear MA | Sh | CAGR | MaxDD | Calmar | H1 | H2 |
|---------|-----|------|-------|--------|-----|-----|
| bear35 | +1.408 | +71.8% | −31.4% | 2.289 | +1.397 | +1.450 |
| bear40 | +1.409 | +71.9% | −31.4% | 2.289 | +1.403 | +1.445 |
| bear50 | +1.399 | +71.4% | −31.4% | 2.277 | +1.392 | +1.437 |
| bear55 | +1.412 | +72.6% | −31.4% | 2.314 | +1.409 | +1.444 |
| **bear60 ✅** | **+1.422** | **+73.4%** | **−31.4%** | **2.340** | **+1.426** | **+1.449** |
| bear70 | +1.400 | +71.9% | −32.6% | 2.202 | +1.414 | +1.417 |
| bear100（原） | 见基准 | — | — | — | — | — |

bear60 是真实峰值（两侧均回落，70 开始 MaxDD 变差）。且 H2(1.449) > H1(1.426)，说明不是对 H1 过拟合。

**第五轮：i2i 精细扫描**（bear60 + CF3 vol>0.04 + mom>0.10）

| i2i 阈值 | Sh | MaxDD | Calmar | H2 |
|----------|-----|-------|--------|-----|
| 0.46 | +1.381 | −32.4% | 2.169 | +1.425 |
| 0.47 | +1.413 | −31.4% | 2.323 | +1.436 |
| **0.48 ✅** | **+1.422** | **−31.4%** | **2.340** | **+1.449** |
| 0.49 | +1.430 | −32.7% | 2.268 | +1.462 |
| 0.50 | +1.428 | −33.6% | 2.208 | +1.455 |

**Pareto 分析**：
- i2i=0.48：MaxDD=-31.4%（最好），Calmar=2.340（最高），全面优势
- i2i=0.49：Sh=1.430（略高 +0.008），MaxDD=-32.7%（略差），H2=1.462（略高）

**最终选定**：i2i=0.48（MaxDD 最优，Calmar 最优，0.008 Sharpe 差异在噪声带内）

**最终最优配置（2026-06-08）**：

| 参数 | 值 | 说明 |
|------|-----|------|
| bear 判定 | **SPY < 60日MA** | 中期熊市判定，比 100MA 更敏感 |
| i2i 阈值 | **≥ 0.48** | 孤立票过滤（前50%截面）|
| CF1 | RSI<30 & mom_21<**−0.15** | 超跌反弹陷阱（放宽捕获 GNRC 类）|
| CF2 | mom_accel<−0.10 & sharpe_10<**−2.5** | 趋势衰竭陷阱（收紧减少误拦）|
| CF3 | **vol_20>0.04 & mom_63>0.10** | 高波动动量反转陷阱（新增）|

**最终对比原基准**：

| 指标 | 原基准（A3+i2i+CF1+CF2, bear100） | 新最优（+CF3+bear60, 精调） | 改善 |
|------|---------|---------|------|
| Sharpe | 1.303 | **1.422** | **+0.119（+9.1%）** |
| CAGR | +64.8% | **+73.4%** | **+8.6pp** |
| MaxDD | −39.5% | **−31.4%** | **−8.1pp（提升）** |
| Calmar | 1.642 | **2.340** | **+0.698（+42.5%）** |
| H1 Sharpe | +1.221 | **+1.426** | **+0.205** |
| H2 Sharpe | +1.426 | **+1.449** | **+0.023** |

**H2 > H1 证明无 H1 过拟合。** CF3 vol=0.04、bear60、i2i=0.48 均为真实峰值（双侧验证），改善幅度（+0.119 Sharpe）远超噪声带（~0.05-0.10）。
...
1. ~~**CF1/CF2 漏网 case 修补**~~ **✅ 2026-06-07 完成**：CF1 mom_21 -0.25→-0.15，CF2 sharpe_10 -2.0→-2.5，Sh +0.042 三指标同升。

2. ~~**bear MA + i2i + CF3 精调**~~ **✅ 2026-06-08 完成**：bear60MA + i2i0.48 + CF3(vol>0.04, mom>0.10)，Sh 1.345→1.422，MaxDD -37.6%→-31.4%，Calmar 1.802→2.340。

3. ~~**宏观跨资产特征**~~ **❌ 2026-06-07 验证失败**：见下方 MaxDD 深度压缩失败实验。

3. ~~**Boost 用在 A2 raw 路径**~~ **❌ 2026-06-07 验证失败**：A2 raw 因 distribution shift 全面落后 A3（Sh 0.917 vs 1.303），mh_boost 无增量。CS-norm 是框架核心不可绕过。

4. ~~**正交 alpha 源：EDGAR 硬过滤器**~~ **❌ 2026-06-07 验证失败**：COVID 亏损票（NVDA/AMD/AAPL）P/E 正常（9x-52x），pe_ttm>200 过滤器完全不触及 MaxDD 窗口的亏损票；MaxDD 分毫未动（-39.5% → -39.5%）。

5. **多 Horizon Boost 与新最优配置联调**：当前 mh_boost 从未在 bear60+CF3 的 mask 下测试，可能有增量（但预计落噪声带内，优先级低）。

6. **CF4 bad case 分析**：新最优配置下最亏的 15 笔单子是否有新的规律？按 bad case 方法论再跑一轮 error analysis，看是否有第四层 CF 的空间。

7. **模型升级**：TabTransformer / LightGBM + Neural reranker 二阶段。当前模型/i2i 侧边际收益已低，此项高成本高不确定性。

8. **降 MaxDD 的结构性手段**（接受需新账户）：当前 -31.4% 已大幅改善。进一步压到 -25% 以内需引入 SPY 认沽期权 / Vol Targeting（已被 CLAUDE.md 明确拒绝作为 MaxDD 手段）。
...
| **宏观跨资产特征** | spy_vol_20 / breadth_200ma / HYG 等 6 个宏观 z-score 特征 | Sh −0.36, MaxDD +10pp 更差 | COVID 是 OOD 样本，训练集无类似极端 regime；全 regime 下都是噪声 |
| **A2 raw + mh_boost** | A2-54f raw（保留绝对尺度）+ mh_swing_boost | Sh 0.917→0.936（远低于 A3 1.303）| CS-norm 是核心：raw 特征 distribution shift 跨年，H1 Sh=0.583 vs A3 1.221 |
| **EDGAR 基本面硬过滤器** | pe_ttm>200 / 负净利润 → 剔出候选 | MaxDD 分毫未动（-39.5%→-39.5%）| COVID 亏损票 P/E 正常（9x-52x），过滤器完全不触及系统性崩盘 |
